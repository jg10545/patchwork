...
def _build_logits(q, k, buffer, tape, N=0, s=0, margin=0):
    """
    ...
    if (N > 0)&(s > 0):
        ...
        # and concatenate on the end of our block'o'logits: (batch_size, buffer_size+s+1)
        all_logits = tf.concat([all_logits, hnm_logits], axis=1)
    return all_logits


def _build_momentum_contrast_training_step(model, mo_model, optimizer, buffer, batches_in_buffer, alpha=0.999, tau=0.07, weight_decay=0, N=0, s=0, margin=0):
    """
    ...
    def training_step(img1, img2, step):
        ...
        with tf.GradientTape() as tape:
            ...
            all_logits = _build_logits(q, k, buffer, tape, N, s, margin)
