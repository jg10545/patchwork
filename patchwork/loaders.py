...
def _build_load_function(path, imshape, norm, num_channels, single_channel,
    ...
    def _load_img(x,y):
        ...
        if augment:
            img = augment_function(imshape, num_channels=num_channels, params=augment)(img)
...
def _image_file_dataset(fps, ys=None, imshape=(256,256),
    ...
    # DIRECTORY OF TFRECORD FILES CASE
        ...
        if augment:
            augment = augment_function(imshape, num_channels=num_channels, params=augment)
...
def _fixmatch_unlab_dataset(fps, weak_aug, str_aug, imshape=(256,256),
    ...
    """
    _weakaug = augment_function(imshape, num_channels, weak_aug)
    _strongaug = augment_function(imshape, num_channels, str_aug)
    def aug_pair(x):
